// // Frontend Code Enhancement:
// // Display Attack Prevention Methods: Add a section in the frontend to display attack prevention methods.

// // Fetch and Display Attack Stats: Fetch attack statistics from the backend and display them.

// // Log Visualization: Display logs fetched from the backend.

// import React, { useState, useEffect } from 'react';
// import { Button, Container, Typography, CircularProgress, Tabs, Tab, Box, Select, MenuItem, FormControl, InputLabel, Grid, Paper, AppBar, Toolbar, List, ListItem, ListItemText } from '@mui/material';
...
//       dns_spoofing_percentage: 0,
//       https_spoofing_percentage: 0,
//       port_scanning_percentage: 0,
//       ddos_percentage: 0,
//       brute_force_percentage: 0,
...
//     };
//     let mitmDetails;
//     // Now you can use mitmDetails in your code    
...
//             <Button variant="contained" onClick={fetchThreatIntelligence}>Fetch Threat Intelligence</Button>
...
import React, { useState, useEffect } from 'react';
import { Container, Typography, Tabs, Tab, Grid, Button, Paper, FormControl, Select, MenuItem, CircularProgress, List, ListItem, ListItemText, InputLabel } from '@mui/material';
import { createTheme, ThemeProvider } from '@mui/material/styles';
import { FaNetworkWired, FaDownload, FaLock, FaChartLine, FaCogs, FaTasks, FaFileAlt } from 'react-icons/fa';
import { Line } from 'react-chartjs-2';
import axios from 'axios';
...
const App = () => {
  ...
  const downloadReport = () => {
    const csvContent = [
      ["Source", "Destination", "Details"],
      ...analysisData.map(detail => [detail.ip_src, detail.ip_dst, detail.details])
    ]
    .map(e => e.join(","))
    .join("\n");
    ...
    const link = document.createElement('a');
    link.setAttribute('href', url);
    link.setAttribute('download', 'Attack_Report.csv');
    link.click();
  };
